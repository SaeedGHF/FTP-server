...
public:
    ...
    int connect(string IP,
                int port);//IP is the ip address of the server to which the clients connect to. Port is the control port
    string getInput();
    string getOwnIP(Socket);    //To get the client's own IP
    int sendRequest();
    char *calculateSize(
            long size);//To calculate the size of the file in appropriate format (like KB, MB etc. in place of bytes)
    ...
    string reduce(const string &str, const string &fill = " ",
                  const string &whitespace = " \t");  //To trim a string internally and externally
