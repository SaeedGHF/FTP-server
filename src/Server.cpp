...
void Server::setDataSocket(Socket socket) {
    this->d_socket = socket;
}

int Server::sendData(string str) {
    if (DEBUG)
        cout << "Sending data: " << str << endl;
    return d_socket.sendMsg(str, 0);
}

int Server::sendData(char *data, int num_bytes) {
...
int Server::sendControl(string str) {
    if (DEBUG)
        cout << "Sending msg: " << str << endl;
    return c_socket.sendMsg(str, 0);
}

string Server::recvControl(int toRead) {
...
int Server::start() {
    ...
    stringstream msg;
    msg << "[SERVER] Successfully started listening on PORT: " << socket.getSocket() << endl;
    Util::log(msg.str());
    return 0;
}
...
int Server::sendResponse() {
    ...
    if (cmd.substr(0, 3) == "user") {
        cout << "User " << cmd.substr(4).c_str() << " umade\n";
        ...
        pf = popen(cmd.append(" 2>&1").c_str(),
                   "r");    //popen does not return stderr file handle. Only stdout. So we are redirecting stderr to stdout
        if (!pf) {
            sendData(" ");
            sendControl("Error occurred while opening pipe for output, calling fork or allocating memory.\n");
            cout << "Command could NOT be successfully executed for client\n";
...
int Server::receiveFile(string filename) {
...
int Server::transferFile(string filename) {
...
int Server::fileExists(const char *filename) {
...
int Server::isEmpty(string filename) {
