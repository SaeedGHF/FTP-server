...
Socket Client::getControlSocket(){
	return c_socket;
}

Socket Client::getDataSocket(){
	return d_socket;
}

void Client::setControlSocket(Socket socket){
	this->c_socket = socket;
}

void Client::setDataSocket(Socket socket){
	this->d_socket = socket;
}

int Client::sendData(char* data, int num_bytes){
...
int Client::sendControl(string str){
	if(DEBUG)
		cout << "Sending msg: " << str << endl;
	return c_socket.sendMsg(str, 0);
}

int Client::recvData(char* data){
...
string Client::recvControl(int toRead){
...
int Client::transferFile(string filename){
...
int Client::receiveFile(string filename){
...
int Client::connect(string serverIP, int controlPort){
...
string Client::getInput(){//takes the input
...
string Client::reduce(const string& str,
                   const string& fill,
                   const string& whitespace) {
    // trim first
...
char * Client::calculateSize(long size)
{   
...
int Client::isEmpty(string filename){
...
int Client::fileExists(const char* filename){
